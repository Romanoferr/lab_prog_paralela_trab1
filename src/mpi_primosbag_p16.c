...
int main(int argc, char *argv[]) {
    ...
    if (meu_ranque == 0) { 
        ...
        MPI_Request *send_requests = (MPI_Request *)malloc((num_procs - 1) * sizeof(MPI_Request));
        ...
        for (dest=1, inicio=3; dest < num_procs && inicio < n; dest++, inicio += TAMANHO) {
            ...
            MPI_Isend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &send_requests[dest-1]);
        }
        ...
        while (active_workers > 0) {
            ...
            MPI_Isend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &send_requests[index]);
            inicio += TAMANHO;
        }
