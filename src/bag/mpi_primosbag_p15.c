...
/* Fica recebendo as contagens parciais de cada processo */
        while (stop < (num_procs-1)) {
            MPI_Irecv(&cont, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &request_mestre);
            MPI_Wait(&request_mestre, &estado);
...
/* Envia um nvo pedaço com TAMANHO números para o mesmo processo*/
        ...
        MPI_Request request_escravo;
        
/* Cada processo escravo recebe o início do espaço de busca */
        while (estado.MPI_TAG != 99) {
            MPI_Irecv(&inicio, 1, MPI_INT, raiz, MPI_ANY_TAG, MPI_COMM_WORLD, &request_escravo);
            MPI_Wait(&request_escravo, &estado);
