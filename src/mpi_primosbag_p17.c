...
int main(int argc, char *argv[]) {
    ...
    if (meu_ranque == 0) { 
        ...
        for (dest=1, inicio=3; dest < num_procs && inicio < n; dest++, inicio += TAMANHO) {
            ...
            MPI_Recv(&ready_signal, 1, MPI_INT, dest, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
            
            // Send work using ready send since we know receive is posted
        ...
        while (active_workers > 0) {
            ...
            // Wait for ready signal before sending more work
            MPI_Recv(&ready_signal, 1, MPI_INT, dest, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
            MPI_Rsend(&inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);
            inicio += TAMANHO;
        }
        ...
        // Worker processes
        while (1) {
            // Post receive for work
            MPI_Irecv(&inicio, 1, MPI_INT, raiz, MPI_ANY_TAG, MPI_COMM_WORLD, &recv_request);
            
            // Signal master that receive is posted
            MPI_Send(&ready_signal, 1, MPI_INT, raiz, tag, MPI_COMM_WORLD);
            
            // Wait for work to arrive
            MPI_Wait(&recv_request, &estado);
            
            if (estado.MPI_TAG == 99) break;  // No more work
