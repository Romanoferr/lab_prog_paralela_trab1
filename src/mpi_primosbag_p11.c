...
int main(int argc, char *argv[]) {
    ...
    if (meu_ranque == 0) { 
        // Master process distributes work using non-blocking send
        for (dest=1, inicio=3; dest < num_procs && inicio < n; dest++, inicio += TAMANHO) {
            ...
            MPI_Wait(&request, &estado); // Wait for send to complete before modifying inicio
        }
        ...
        while (estado.MPI_TAG != 99) {
            ...
            if (estado.MPI_TAG != 99) {
                ...
                MPI_Wait(&request, &estado); // Ensure send completes before next iteration
            } 
        } 
    }
